...
service_start() {
    ...
    # 检查mihomo是否安装
    local mihomo_bin=$(which mihomo 2>/dev/null || echo "")
    ...
    # 启动服务
    if sudo systemctl start mihomo-tun.service; then
        log_success "服务启动成功"
        
        # 启用自启动
        if sudo systemctl enable mihomo-tun.service; then
            log_success "已启用自启动"
        else
            log_warning "启用自启动失败"
        fi
        ...
        log_error "服务启动失败"
        log_info "请查看日志: journalctl -u mihomo-tun.service -f"
    fi
...
service_stop() {
    ...
    if sudo systemctl stop mihomo-tun.service; then
        log_success "服务停止成功"
        
        # 禁用自启动
        if sudo systemctl disable mihomo-tun.service; then
            log_success "已禁用自启动"
        else
            log_warning "禁用自启动失败"
        fi
...
service_restart() {
    ...
    if sudo systemctl restart mihomo-tun.service; then
        log_success "服务重启成功"
    else
        log_error "服务重启失败"
    fi
...
service_status() {
    ...
    # 检查是否启用
    if systemctl is-enabled --quiet mihomo-tun.service 2>/dev/null; then
    ...
    echo ""
    systemctl status mihomo-tun.service --no-pager
...
service_logs() {
    ...
    # 显示日志
    journalctl -u mihomo-tun.service -n 50 --no-pager
    
    echo ""
    echo -e "${YELLOW}提示: 使用 'journalctl -u mihomo-tun.service -f' 查看实时日志${NC}"
    echo ""
...
# 创建systemd服务文件
create_service_file() {
    local service_file="/etc/systemd/system/mihomo-tun.service"
    local config_file="${CONFIGS_DIR}/config.yaml"
    
    # 获取mihomo路径
    local mihomo_bin=$(which mihomo 2>/dev/null || echo "/usr/local/bin/mihomo")
